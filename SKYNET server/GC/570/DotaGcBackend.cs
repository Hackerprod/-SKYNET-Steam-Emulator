...
public sealed partial class DotaGcBackend : ILuaGameCoordinatorBackend
{
    ...
    public string DotaPatchLobbyReplayPayload(
        ...
        string lobbyName)
    {
        var members = ParseReplayMembers(players);
        var options = new LobbyReplayPatchOptions(
            ParseUInt64(syncVersion),
            ParseUInt64(lobbyId),
            ParseUInt64(leaderSteamId),
            ParseUInt64(serverSteamId),
            ParseUInt64(matchId),
            connect ?? string.Empty,
            lobbyState,
            gameState,
            gameStartTime,
            string.IsNullOrWhiteSpace(lobbyName) ? "Sala 1" : lobbyName,
            members);

        return Encode(PatchLobbyReplayEnvelope(Decode(payloadBase64), options));
    }
    ...
    private static byte[] PatchLobbyReplayEnvelope(byte[] payload, LobbyReplayPatchOptions options)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
            if (field.Number == 2 && field.WireType == 2)
            {
                field.BytesValue = PatchReplaySubscribedType(field.BytesValue, options);
            }
            else if (field.Number == 3 && field.WireType == 1)
            {
                field.Fixed64Value = options.SyncVersion;
            }
            else if ((field.Number == 4 || field.Number == 6) && field.WireType == 2)
            {
                field.BytesValue = PatchReplayOwnerSoid(field.BytesValue, options.LobbyId);
            }
        }

        return WriteReplayFields(fields);
    }

    private static byte[] PatchReplaySubscribedType(byte[] payload, LobbyReplayPatchOptions options)
    {
        var fields = ReadReplayFields(payload);
        uint objectType = 0;
        foreach (var field in fields)
        {
            ...
            {
                objectType = (uint)field.VarintValue;
                break;
            }
        }
        ...
        foreach (var field in fields)
        {
            ...
            field.BytesValue = objectType switch
            {
                LobbyObjectTypeId => PatchReplayLobbyObject(field.BytesValue, options),
                LobbyPersonaObjectTypeId => PatchReplayPersonaObject(field.BytesValue, options.Members),
                LobbyBroadcastObjectTypeId => PatchReplayBroadcastObject(field.BytesValue, options.Members),
                LobbyMemberObjectTypeId => PatchReplayMemberSummaryObject(field.BytesValue, options.Members),
                _ => field.BytesValue
            };
        }
    ...
    private static byte[] PatchReplayLobbyObject(byte[] payload, LobbyReplayPatchOptions options)
    {
        var fields = ReadReplayFields(payload);
        int memberIndex = 0;
        int memberIndexField = 0;
        var output = new List<ReplayField>(fields.Count);
        ...
        foreach (var field in fields)
        {
            switch (field.Number)
            {
                case 1 when field.WireType == 0:
                    field.VarintValue = options.LobbyId;
                    break;
                case 4 when field.WireType == 0:
                    field.VarintValue = options.LobbyState;
                    break;
                case 5 when field.WireType == 2:
                    field.BytesValue = System.Text.Encoding.UTF8.GetBytes(options.Connect);
                    break;
                case 6 when field.WireType == 1:
                    field.Fixed64Value = options.ServerSteamId;
                    break;
                case 11 when field.WireType == 1:
                    field.Fixed64Value = options.LeaderSteamId;
                    break;
                case 16 when field.WireType == 2:
                    field.BytesValue = System.Text.Encoding.UTF8.GetBytes(options.LobbyName);
                    break;
                case 28 when field.WireType == 0:
                    field.VarintValue = options.GameState;
                    break;
                case 30 when field.WireType == 0:
                    field.VarintValue = options.MatchId;
                    break;
                case 87 when field.WireType == 0:
                    field.VarintValue = options.GameStartTime;
                    break;
                case 120 when field.WireType == 2:
                    if (memberIndex >= options.Members.Count)
                    {
                        continue;
                    }

                    field.BytesValue = PatchReplayLobbyMember(field.BytesValue, options.Members[memberIndex]);
                    memberIndex++;
                    break;
                case 121 when field.WireType == 0:
                    if (memberIndexField >= options.Members.Count)
                    {
                        continue;
                    }

                    field.VarintValue = (ulong)memberIndexField;
                    memberIndexField++;
                    break;
                case 128 when field.WireType == 0:
                    field.VarintValue = (ulong)DateTimeOffset.UtcNow.ToUnixTimeSeconds();
                    break;
            }
        ...
        return WriteReplayFields(output);
    }

    private static byte[] PatchReplayLobbyMember(byte[] payload, ReplayLobbyMember member)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
            switch (field.Number)
            {
                case 1 when field.WireType == 1:
                    field.Fixed64Value = member.SteamId;
                    break;
                case 3 when field.WireType == 0:
                    field.VarintValue = member.Team;
                    break;
                case 7 when field.WireType == 0:
                    field.VarintValue = member.Slot;
                    break;
                case 8 when field.WireType == 0 && member.HeroId != 0:
                    field.VarintValue = member.HeroId;
                    break;
                case 23 when field.WireType == 0:
                    field.VarintValue = member.CoachTeam;
                    break;
            }
        }
    ...
    private static byte[] PatchReplayPersonaObject(byte[] payload, IReadOnlyList<ReplayLobbyMember> members)
    {
        var fields = ReadReplayFields(payload);
        int index = 0;
        var output = new List<ReplayField>(fields.Count);
        foreach (var field in fields)
        {
            if (field.Number == 1 && field.WireType == 2)
            {
                ...
                field.BytesValue = PatchReplayPersonaEntry(field.BytesValue, members[index].Name);
                index++;
            }
    ...
    private static byte[] PatchReplayPersonaEntry(byte[] payload, string name)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
            if (field.Number == 1 && field.WireType == 2)
            {
                field.BytesValue = System.Text.Encoding.UTF8.GetBytes(name ?? string.Empty);
            }
        }
    ...
    private static byte[] PatchReplayBroadcastObject(byte[] payload, IReadOnlyList<ReplayLobbyMember> members)
    {
        var fields = ReadReplayFields(payload);
        int memberIndex = 0;
        foreach (var field in fields)
        {
            if (field.Number == 2 && field.WireType == 2 && memberIndex < members.Count)
            {
                field.BytesValue = PatchReplayExtraMessage(field.BytesValue, members[memberIndex]);
                memberIndex++;
            }
        }
    ...
    private static byte[] PatchReplayExtraMessage(byte[] payload, ReplayLobbyMember member)
    {
        var fields = ReadReplayFields(payload);
        uint messageType = 0;
        foreach (var field in fields)
        {
            if (field.Number == 1 && field.WireType == 0)
            {
                messageType = (uint)field.VarintValue;
                break;
            }
        }
        ...
        if (messageType == 8869)
        {
            foreach (var field in fields)
            {
                if (field.Number == 2 && field.WireType == 2)
                {
                    field.BytesValue = PatchReplayAdditionalAccountData(field.BytesValue, member.AccountId);
                }
            }
        }
    ...
    private static byte[] PatchReplayAdditionalAccountData(byte[] payload, uint accountId)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
    ...
    private static byte[] PatchReplayMemberSummaryObject(byte[] payload, IReadOnlyList<ReplayLobbyMember> members)
    {
        var fields = ReadReplayFields(payload);
        int memberIndex = 0;
        var output = new List<ReplayField>(fields.Count);
        foreach (var field in fields)
        {
            if (field.Number == 1 && field.WireType == 2)
            {
                ...
                field.BytesValue = PatchReplayMemberSummaryEntry(field.BytesValue, members[memberIndex]);
                memberIndex++;
            }
            else if (field.Number == 3 && field.WireType == 2 && members.Count > 0)
            {
                field.BytesValue = PatchReplayLobbyEventPoints(field.BytesValue, members);
            }
    ...
    private static byte[] PatchReplayMemberSummaryEntry(byte[] payload, ReplayLobbyMember member)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
            if (field.Number == 1 && field.WireType == 1)
            {
                field.Fixed64Value = member.SteamId;
            }
        }
    ...
    private static byte[] PatchReplayLobbyEventPoints(byte[] payload, IReadOnlyList<ReplayLobbyMember> members)
    {
        var fields = ReadReplayFields(payload);
        int memberIndex = 0;
        var output = new List<ReplayField>(fields.Count);
        foreach (var field in fields)
        {
            if (field.Number == 2 && field.WireType == 2)
            {
                ...
                field.BytesValue = PatchReplayLobbyEventPointsMember(field.BytesValue, members[memberIndex].AccountId);
                memberIndex++;
            }
    ...
    private static byte[] PatchReplayLobbyEventPointsMember(byte[] payload, uint accountId)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
    ...
    private static byte[] PatchReplayOwnerSoid(byte[] payload, ulong lobbyId)
    {
        var fields = ReadReplayFields(payload);
        foreach (var field in fields)
        {
    ...
    private static List<ReplayLobbyMember> ParseReplayMembers(string players)
    {
        var members = new List<ReplayLobbyMember>();
        foreach (var line in (players ?? string.Empty).Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            members.Add(new ReplayLobbyMember(
                ParseUInt64(parts[0]),
                uint.TryParse(parts[1], out var account) ? account : 0,
                parts[2],
                uint.TryParse(parts[3], out var team) ? team : 0,
                uint.TryParse(parts[4], out var slot) ? slot : 0,
                uint.TryParse(parts[5], out var coach) ? coach : 5,
                uint.TryParse(parts[6], out var hero) ? hero : 0));
        }

        if (members.Count == 0)
        {
            members.Add(new ReplayLobbyMember(0, 0, "User", 0, 1, 5, 0));
        }

        return members;
    }

    private static List<ReplayField> ReadReplayFields(byte[] payload)
    {
        var fields = new List<ReplayField>();
        int index = 0;
        while (payload != null && index < payload.Length)
        {
            if (!TryReadVarint(payload, ref index, out ulong tag))
            {
                break;
            }
            ...
            var field = new ReplayField(number, wireType);
            switch (wireType)
            {
                case 0:
                    if (!TryReadVarint(payload, ref index, out ulong varintValue))
                    {
                        return fields;
                    }
                    field.VarintValue = varintValue;
                    break;
                case 1:
                    ...
                    field.Fixed64Value = BitConverter.ToUInt64(payload, index);
                    index += 8;
                    break;
                case 2:
                    if (!TryReadVarint(payload, ref index, out ulong length) || index + (int)length > payload.Length)
                    {
                        return fields;
                    }
                    field.BytesValue = new byte[(int)length];
                    Array.Copy(payload, index, field.BytesValue, 0, field.BytesValue.Length);
                    index += field.BytesValue.Length;
                    break;
                case 5:
                    ...
                    field.Fixed32Value = BitConverter.ToUInt32(payload, index);
                    index += 4;
                    break;
                default:
                    return fields;
    ...
    private static byte[] WriteReplayFields(IEnumerable<ReplayField> fields)
    {
        ...
        foreach (var field in fields)
        {
            ...
            {
                case 0:
                    WriteVarintField(output, field.Number, field.VarintValue);
                    break;
                case 1:
                    WriteFixed64Field(output, field.Number, field.Fixed64Value);
                    break;
                case 2:
                    WriteBytesField(output, field.Number, field.BytesValue);
                    break;
                case 5:
                    WriteFixed32Field(output, field.Number, field.Fixed32Value);
                    break;
            }
        }
    ...
    private sealed record LobbyReplayPatchOptions(
        ulong SyncVersion,
        ulong LobbyId,
        ulong LeaderSteamId,
        ulong ServerSteamId,
        ...
        uint GameStartTime,
        string LobbyName,
        IReadOnlyList<ReplayLobbyMember> Members);

    private sealed record ReplayLobbyMember(
        ulong SteamId,
        uint AccountId,
        string Name,
        uint Team,
    ...
    private sealed class ReplayField
    {
        public ReplayField(int number, int wireType)
        {
            Number = number;
            WireType = wireType;
