...
namespace SKYNET.Wave
{
    ...
    public class WaveIn
    {
        ...
        public WaveIn(WavInDevice device,int samplesPerSec,int bitsPerSample,int channels,int bufferSize)
        {
            ...
            {
                Log.Write("AudioManager", $"Failed to open wav device, error: {(MMSYSERR)result}");
            }
        ...
        public void Start()
        {
            ...
            if(result != (int)MMSYSERR.NOERROR){
                Log.Write("AudioManager", $"Failed to start wav device, error: {(MMSYSERR)result}");
            }
        }
        ...
        public void Stop()
        {
            ...
            if(result != (int)MMSYSERR.NOERROR){
                Log.Write("AudioManager", $"Failed to stop wav device, error: {(MMSYSERR)result}");
            }
        }
            ...
            lock(m_pBuffers){
                while(m_pBuffers.Count < 3){
                    ...
                    if(result == (int)MMSYSERR.NOERROR){
                        ...
                        if(result != (int)MMSYSERR.NOERROR){
                            Log.Write("AudioManager", $"Error adding wave in buffer, error: {(MMSYSERR)result}");
                        }
                    }
                }
            }
