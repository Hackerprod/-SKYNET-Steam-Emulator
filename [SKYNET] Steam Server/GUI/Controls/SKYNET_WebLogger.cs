...
namespace SKYNET.GUI.Controls
{
    ...
    public partial class SKYNET_WebLogger : UserControl
    {
        ...
        private void InitializeWebBrowser()
        {
            ...
            webChat.Document.OpenNew(true).Write($"<html><head>{GetHtmlIEVersion()}{ScrollBar(_scroll) + JavaScript()} <style>{GetStyles()}</style>  <title name = 'head'>SKYNET</title>" + $"</head><body class='body' bgcolor={ColorTranslator.ToHtml(Color.FromArgb(29, 39, 51))}><table style=\"padding: 0px\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" id='table'>");
            AssignStyleSheet();
