...
namespace SKYNET.Interface
{
    public interface ISteamUser
    {
        ...
        int InitiateGameConnection(IntPtr pAuthBlob, int cbMaxAuthBlob, IntPtr steamIDGameServer, uint unIPServer, uint usPortServer, bool bSecure);

        // notify of disconnect
        // needs to occur when the game client leaves the specified game server, needs to match with the InitiateGameConnection() call
        void TerminateGameConnection(uint unIPServer, uint usPortServer);

        // Legacy functions

        // used by only a few games to track usage events
        void TrackAppUsageEvent(IntPtr gameID, int eAppUsageEvent, string pchExtraInfo = "");

        // get the local storage folder for current Steam account to write application data, e.g. save games, configs etc.
        // this will usually be something like "C:\Progam Files\Steam\userdata\<SteamID>\<AppID>\local"
        bool GetUserDataFolder(string pchBuffer, int cubBuffer);
        ...
        EVoiceResult GetAvailableVoice(uint pcbCompressed, uint pcbUncompressed_Deprecated, uint nUncompressedVoiceDesiredSampleRate_Deprecated);
        ...
        EVoiceResult DecompressVoice(IntPtr pCompressed, uint cbCompressed, IntPtr pDestBuffer, uint cbDestBufferSize, uint nBytesWritten, uint nDesiredSampleRate);
        ...
        HAuthTicket GetAuthSessionTicket(IntPtr pTicket, int cbMaxTicket, uint pcbTicket);

        // Authenticate ticket from entity steamID to be sure it is valid and isnt reused
        // Registers for callbacks if the entity goes offline or cancels the ticket ( see ValidateAuthTicketResponse_t callback and EAuthSessionResponse )
        EBeginAuthSessionResult BeginAuthSession(IntPtr pAuthTicket, int cbAuthTicket, IntPtr steamID);

        // Stop tracking started by BeginAuthSession - called when no longer playing game with this entity
        void EndAuthSession(IntPtr steamID);
        ...
        EUserHasLicenseForAppResult UserHasLicenseForApp(IntPtr steamID, AppId_t appID);
        ...
        // IntPtr steamIDGameServer - the steamID of the game server, received from the game server by the client
        // uint unIPServer, uint16 usPortServer - the IP address of the game server
        void AdvertiseGame(IntPtr steamIDGameServer, uint unIPServer, uint usPortServer);

        // Requests a ticket encrypted with an app specific shared key
        // pDataToInclude, cbDataToInclude will be encrypted into the ticket
        // ( This is asynchronous, you must wait for the ticket to be completed by the server )

        SteamAPICall_t RequestEncryptedAppTicket(IntPtr pDataToInclude, int cbDataToInclude);
        ...
        bool GetEncryptedAppTicket(IntPtr pTicket, int cbMaxTicket, uint pcbTicket);
        ...
        SteamAPICall_t RequestStoreAuthURL(string pchRedirectURL);
