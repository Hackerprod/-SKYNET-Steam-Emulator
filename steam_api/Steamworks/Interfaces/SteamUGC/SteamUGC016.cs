...
namespace SKYNET.Interface
{
    ...
    public class SteamUGC016 : ISteamInterface
    {
        ...
        public uint GetQueryUGCNumTags(IntPtr _, ulong handle, uint index)
        {
            return SteamEmulator.SteamUGC.GetQueryUGCNumTags(handle, index);
        }

        public bool GetQueryUGCTag(IntPtr _, ulong handle, uint index, uint indexTag, string pchValue, uint cchValueSize)
        {
            return SteamEmulator.SteamUGC.GetQueryUGCTag(handle, index, indexTag, pchValue, cchValueSize);
        }

        public bool GetQueryUGCTagDisplayName(IntPtr _, ulong handle, uint index, uint indexTag, string pchValue, uint cchValueSize)
        {
        ...
        public bool SetTimeCreatedDateRange(IntPtr _, ulong handle, IntPtr rtStart, IntPtr rtEnd)
        {
            return SteamEmulator.SteamUGC.SetTimeCreatedDateRange(handle, rtStart, rtEnd);
        }

        public bool SetTimeUpdatedDateRange(IntPtr _, ulong handle, IntPtr rtStart, IntPtr rtEnd)
        {
        ...
        public bool SetItemTitle(IntPtr _, ulong handle, string pchTitle)  // change the title of an UGC item
        {
            return SteamEmulator.SteamUGC.SetItemTitle(handle, pchTitle);
        }

        public bool SetItemDescription(IntPtr _, ulong handle, string pchDescription)  // change the description of an UGC item
        {
            return SteamEmulator.SteamUGC.SetItemDescription(handle, pchDescription);
        }

        public bool SetItemUpdateLanguage(IntPtr _, ulong handle, string pchLanguage)  // specify the language of the title or description that will be set
        {
        ...
        public bool SetItemContent(IntPtr _, ulong handle, string pszContentFolder)  // update item content from this local folder
        {
            return SteamEmulator.SteamUGC.SetItemContent(handle, pszContentFolder);
        }

        public bool SetItemPreview(IntPtr _, ulong handle, string pszPreviewFile)  //  change preview image file for this item. pszPreviewFile points to local image file, which must be under 1MB in size
        {
        ...
        public bool RemoveAllItemKeyValueTags(IntPtr _, ulong handle)  // remove all existing key-value tags (IntPtr _, you can add new ones via the AddItemKeyValueTag function)
        {
            return SteamEmulator.SteamUGC.RemoveAllItemKeyValueTags(handle);
        }

        public bool RemoveItemKeyValueTags(IntPtr _, ulong handle, string pchKey)  // remove any existing key-value tags with the specified key
        {
            return SteamEmulator.SteamUGC.RemoveItemKeyValueTags(handle, pchKey);
        }

        public bool AddItemKeyValueTag(IntPtr _, ulong handle, string pchKey, string pchValue)  // add new key-value tags for the item. Note that there can be multiple values for a tag.
        {
            return SteamEmulator.SteamUGC.AddItemKeyValueTag(handle, pchKey, pchValue);
        }

        public bool AddItemPreviewFile(IntPtr _, ulong handle, string pszPreviewFile, int type)  //  add preview file for this item. pszPreviewFile points to local file, which must be under 1MB in size
        {
            return SteamEmulator.SteamUGC.AddItemPreviewFile(handle, pszPreviewFile, type);
        }

        public bool AddItemPreviewVideo(IntPtr _, ulong handle, string pszVideoID)  //  add preview video for this item
        {
            return SteamEmulator.SteamUGC.AddItemPreviewVideo(handle, pszVideoID);
        }

        public bool UpdateItemPreviewFile(IntPtr _, ulong handle, uint index, string pszPreviewFile)  //  updates an existing preview file for this item. pszPreviewFile points to local file, which must be under 1MB in size
        {
            return SteamEmulator.SteamUGC.UpdateItemPreviewFile(handle, index, pszPreviewFile);
        }

        public bool UpdateItemPreviewVideo(IntPtr _, ulong handle, uint index, string pszVideoID)  //  updates an existing preview video for this item
        {
            return SteamEmulator.SteamUGC.UpdateItemPreviewVideo(handle, index, pszVideoID);
        }

        public bool RemoveItemPreview(IntPtr _, ulong handle, uint index)  // remove a preview by index starting at 0 (IntPtr _, previews are sorted)
        {
        ...
        public ulong SubscribeItem(IntPtr _, ulong nPublishedFileID)  // subscribe to this item, will be installed ASAP
        {
            return SteamEmulator.SteamUGC.SubscribeItem(nPublishedFileID);
        }

        public ulong UnsubscribeItem(IntPtr _, ulong nPublishedFileID)  // unsubscribe from this item, will be uninstalled after game quits
        {
        ...
        public ulong GetWorkshopEULAStatus(IntPtr _)
        {
