...
namespace SKYNET.Managers
{
    public static class StateCache
    {
        ...
        public static void ApplySelf(APIClient.ApiUser user)
        {
            if (user == null)
            {
                return;
        ...
        public static void ApplyFriends(IEnumerable<APIClient.ApiUser> friends)
        {
            if (friends == null)
            {
                return;
        ...
        public static void UpsertUser(APIClient.ApiUser user)
        {
            if (user == null)
            {
                return;
        ...
        public static void ApplyStats(ulong steamId, IEnumerable<APIClient.ApiStat> stats)
        {
        ...
        public static void ApplyAchievements(ulong steamId, IEnumerable<APIClient.ApiAchievement> achievements)
        {
        ...
        public static void UpsertFriendFromEvent(APIClient.ApiEvent serverEvent)
        {
            if (serverEvent == null)
            {
                return;
            ...
            if (steamId == 0 || steamId == (ulong)SteamEmulator.SteamID)
            {
                return;
            }

            Friends.TryGetValue(steamId, out var existing);
            ...
            if (relationship == 0)
            {
                if (serverEvent.Type == "friend_added")
                    ...
                    relationship = existing.FriendRelationship != 0
                        ? existing.FriendRelationship
                        : (existing.HasFriend
                            ? (int)EFriendRelationship.k_EFriendRelationshipFriend
                            : (int)EFriendRelationship.k_EFriendRelationshipNone);
            }

            var richPresence = new Dictionary<string, string>(existing?.RichPresence ?? new Dictionary<string, string>());
            if (serverEvent.RichPresence != null)
            {
                richPresence = new Dictionary<string, string>(serverEvent.RichPresence);
            }

            var user = new SteamPlayer
            {
                AccountID = serverEvent.AccountId != 0 ? serverEvent.AccountId : (existing?.AccountID ?? steamId.GetAccountID()),
                SteamID = steamId,
                PersonaName = !string.IsNullOrWhiteSpace(serverEvent.PersonaName)
                    ? serverEvent.PersonaName
        ...
        private static SteamPlayer MapUser(APIClient.ApiUser user)
        {
