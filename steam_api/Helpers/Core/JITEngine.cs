...
namespace Steam4NET.Core
{
    ...
    public static class JITEngine
    {
        ...
        static JITEngine()
        {
            string RandomName = new Random().Next(1000, 9999).ToString();

            AssemblyBuilder assemblyBuilder = AppDomain.CurrentDomain.DefineDynamicAssembly(new AssemblyName(RandomName), AssemblyBuilderAccess.RunAndSave);
...
#endif
            moduleBuilder = assemblyBuilder.DefineDynamicModule(RandomName, true);
        }
        ...
        public static TClass GenerateClass<TClass>(IntPtr ptr) where TClass : class
        {
            ...
            TypeBuilder builder = moduleBuilder.DefineType(RandomName + "_" + (IntPtr.Size * 8).ToString(),
